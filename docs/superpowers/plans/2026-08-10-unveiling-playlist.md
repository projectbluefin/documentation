...
**Files:**
- Generate locally: `static/data/playlist-metadata.json` - ignored build-time metadata.
- Create: `static/img/playlists/PLhiPP9M5fgWETQZheRqv2ue5bv-zZNpK5.jpg` - tracked cached thumbnail.
...
Expected: the script fetches YouTube metadata for the registered playlists and exits successfully, writing the ignored `static/data/playlist-metadata.json` and caching the new thumbnail under `static/img/playlists/`.
...
- [ ] **Step 3: Commit the tracked generated thumbnail**

Run:

```bash
git add static/img/playlists/PLhiPP9M5fgWETQZheRqv2ue5bv-zZNpK5.jpg
git diff --cached --name-only
git commit -m "build(music): cache Unveiling playlist thumbnail" -m "Assisted-by: GPT-5.6 Luna via GitHub Copilot

Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>"
```

Expected: only the new playlist thumbnail is included. The generated JSON remains ignored and is regenerated by the existing build pipeline.

### Task 3: Run final validation

**Files:**
- Test: `scripts/fetch-playlist-metadata.test.js`
- Validate: `docs/music.md`, `scripts/fetch-playlist-metadata.js`, generated `static/data/playlist-metadata.json`, and the generated thumbnail.
...
Expected: the implementation commits contain only the intended source and tracked thumbnail files; pre-existing unrelated working-tree changes remain unstaged and untouched.
