...
The setup script defaults to Gemini CLI as the provider. To use a different one, run `goose configure` to switch.

For local models, [Ramalama](https://github.com/containers/ramalama) is the recommended option — it manages models as containers and serves an OpenAI-compatible endpoint:

```bash
brew install ramalama
ramalama serve llama3.2
# serves at http://localhost:8080
```

[LM Studio](https://lmstudio.ai/) is a graphical alternative (`brew install ublue-os/tap/lm-studio-linux`) that serves at `http://localhost:1234`. For hosted providers (OpenAI, Anthropic, Gemini, etc.), run `goose configure` to set your API key.
